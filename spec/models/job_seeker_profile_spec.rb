...
RSpec.describe JobSeekerProfile, type: :model do
  describe 'バリデーション' do
    ...
    it '姓がなければ無効であること' do
      expect(build(:job_seeker_profile, last_name: '')).to_not be_valid
    end

    it '名がなければ無効であること' do
      expect(build(:job_seeker_profile, first_name: '')).to_not be_valid
    end

    it '姓(ふりがな)がなければ無効であること' do
      expect(
        build(:job_seeker_profile, last_name_furigana: '')
      ).to_not be_valid
    end

    it '名(ふりがな)がなければ無効であること' do
      expect(
        build(:job_seeker_profile, first_name_furigana: '')
      ).to_not be_valid
    end

    it 'bioは160文字以内であること' do
      expect(build(:job_seeker_profile, bio: 'a' * 161)).to_not be_valid
    end
  end
